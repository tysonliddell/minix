...
void build_prefix(char *path1, char *path2, char *path3)
{
   char * newstr;
   int l;
...
void prepend_option(char *option, int otype)
{
   struct opt_list * newopt = xalloc(sizeof(struct opt_list));
...
#endif
   fprintf(out, "Usage: %s [-ansi] [-options] [-o output] file [files].\n", progname);
   exit(status);
}

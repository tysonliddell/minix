...
static int pipe_stdin_through_cpp(char **argv)
{
    int     pipefds[2];
    int     pid;
    char  **cpptr = argv;
...
static void show_arg_names(register struct token *t)
{
...
static void show_arg_types(register struct token *t)
{
    register struct token *s;
...
static void header_flush(register struct token *t)
{
...
static void fpf_header_names(struct token *list)
{
    register struct token *s;
...
static void fpf_header_types(struct token *list)
{
    register struct token *s;
    register struct token *p;
...
static void fpf_header(struct token *l1, struct token *l2)
{
...
static struct token *skip_enclosed(register struct token *p, register int stop)
{
...
static int is_func_ptr_cast(register struct token *t)
{
    register struct token *p;
...
static struct token *func_ptr_dcl_flush(register struct token *list)
{
    register struct token *t;
...
static struct token *function_dcl_flush(register struct token *list)
{
    register struct token *t;

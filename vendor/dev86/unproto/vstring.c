...
/*	#include "vstring.h"
/*
/*	struct vstring *vs_alloc(int len);
/*
/*	int VS_ADDCH(struct vstring *vs, char *wp, int ch)
/*
/*	char *vs_strcpy(struct vstring *vp, char *dst, char *src);
/* DESCRIPTION
...
char *vs_realloc(register struct vstring *vp, char *cp)
{
    int     where = cp - vp->str;
...
char *vs_strcpy(register struct vstring *vp, register char *dst, register char *src)
{
    while (*src) {
